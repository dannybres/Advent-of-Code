...
distances = ones(size(startR));

for idx = 1:numel(startR)
    startNode = "Tree" + startR(idx) + "_" + startC(idx);
    path = shortestpath(d,startNode,bestSignalNode);
    distances(idx) = numel(path)-1;
end

distances = distances(distances ~= -1);

day12puzzle2result = min(distances) %#ok<NOPTS>