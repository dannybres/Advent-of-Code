...
def unscamble(password):
    ...
    for line in lines:
        ...
        if type == "swap":
            ...
            if args[0] == "based":
                amountLeft = [1, 1, 6, 2, 7, 3, 0, 4]
                locationOfLetter = [n for n,x in enumerate(password) if x == args[-1]][0]
                shiftBy = amountLeft[locationOfLetter]
                password = password[shiftBy:] + password[:shiftBy]
            elif args[0] == "right":
                password = password[int(args[1]) % len(password):] + password[:int(args[1]) % len(password)]
            else:
