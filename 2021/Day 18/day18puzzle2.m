%% day18puzzle2 - Daniel Breslan - Advent Of Code 2021
clear, clc
in = readlines("input.txt");
[x,y] = meshgrid(1:numel(in),1:numel(in));
% x = tril(x,-1);
...
for ri = 1:size(x,1)
    ...
    for ci = 1:size(x,2)
        ...
        data = sumAndReduceMe(in(x(ri,ci)) + " + " + in(y(ri,ci)));
        result(ri,ci) = magnitudeCalculator(data);
    end
end
