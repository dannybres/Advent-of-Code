%% day15puzzle2 - Daniel Breslan - Advent Of Code 2019
...
oxygenSystem = map(map(:,3) == 2,1:2);
potentialOxgen = repelem(oxygenSystem,4,1) + ...
    repmat(dirctions,height(oxygenSystem),1);
map(ismember(map(:,1:2),potentialOxgen,"rows") & map(:,3) == 1,3) = 2;
idx = idx + 1;
end
day15puzzle2result = idx %#ok<NOPTS>
